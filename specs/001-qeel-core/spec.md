...
3. **Given** 年次別・パラメータ別の順位相関係数が算出されている、**When** 可視化を実行する、**Then** 分布図が生成され、パラメータのロバスト性が評価できる
...
- データソースに欠損値や異常値が含まれる場合、システムはどのように処理するか？（例: 特定日時のOHLCVデータが欠損、価格がゼロまたは異常に高い値）
- ポジション情報が破損または不整合な状態でiterationが開始された場合、システムはどのように対処するか？
- 実運用で取引所APIがエラーを返した場合（タイムアウト、レート制限、注文拒否など）、システムはどのようにリトライまたは通知するか？
- tomlの設定ファイルが不正な形式または矛盾した設定を含む場合、システムは起動前にバリデーションを行うか？
- 複数銘柄のバックテストで、一部の銘柄のみデータが不足している場合、その銘柄をスキップして続行するか、エラーとするか？
- シグナル計算やリターン計算で数値オーバーフロー、ゼロ除算、無限大が発生した場合、システムはどのように処理するか？
...
- **FR-004**: システムは、iteration内で以下のステップを分離して実装できなければならない：
  ...
  - 銘柄選定（select_symbols）: 入出力はPolars DataFrameを使用
  - 執行条件計算（create_orders）: 入出力はPolars DataFrameを使用
  - 執行（submit_orders）
...
- **FR-017**: システムは、データソースの読み込み元をユーザが柔軟に定義できるインターフェースを提供しなければならない（CSV、Parquet、データベース、APIなど）

#### コンテキスト管理

- **FR-018**: システムは、ポジション情報、選択銘柄、モデルパラメータなどのコンテキストを、CSV、JSON、Parquetから復元可能な形式で保存・読み込みできなければならない
...
- **SC-005**: パラメータグリッド評価において、100通りのパラメータ組み合わせに対して順位相関係数の分布が可視化され、オーバーフィットの有無を判断できる
- **SC-006**: 複数銘柄（例: 100銘柄）のバックテストが、ユーザのローカルマシンで実行可能な速度で完了する（例: 1年分のデータに対して10分以内）
- **SC-007**: データ欠損やAPIエラーが発生した場合でも、システムは適切なエラーメッセージを出力し、復旧可能な状態を保つ
...
- **データ提供**: ユーザが市場データ（OHLCV、決算情報など）を任意の形式（CSV、Parquet、データベース、APIなど）で用意する必要がある
...
- **外部検証済み**: シグナル計算ロジックは、本システムの外部で詳細に検証済みであることを前提とし、バックテストは最終確認として1度のみ実行する
