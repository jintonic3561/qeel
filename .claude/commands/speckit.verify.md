...
1.  Identify **Modified Files**: Extract file paths mentioned in the *completed* tasks within `tasks.md`.
2.  **Read Code**: Load the actual content of these implementation files from the repository.
3.  **Traceability Mapping**:
    *   Map Code Component $\leftrightarrow$ Task ID $\leftrightarrow$ User Story $\leftrightarrow$ Functional Requirement.
...
*   **Functionality**: Does the code actually perform the logic described in the User Story?
*   **Data Integrity**: Do the implemented models/schemas match `data-model.md` exactly? (Check field names, types, constraints).
...
*   **Omissions**: Are there requirements in `spec.md` that should have been implemented in the completed tasks but are missing?
...
*List implementation errors where code contradicts the spec.*
...
*List areas where the code works, but the spec/design was unclear or flawed.*
...
*   **Test Coverage Check**: *Did the plan require tests? Do they exist for the completed files?*
