...
- Q: シグナル計算クラスとリターン計算クラスの設計パターン → A: 抽象基底クラス（ABC）を提供し、ユーザが具体的な計算ロジックをオーバーライドする形式で実装する。これにより一貫したインターフェースと拡張性を確保する。

### Session 2025-11-26

- Q: データ欠損が発生した場合、システムはどのような振る舞いをすべきですか？ → A: NaN/nullのまま処理継続
- Q: 実運用で取引所APIエラー（タイムアウト、レート制限、注文拒否など）が発生した場合、システムはどのように処理すべきですか？ → A: スコープ外（APIラッパーはユーザがオーバーライドする前提のため、エラーハンドリングもユーザ責任）
- Q: toml設定ファイルが不正な形式または矛盾した設定を含む場合、システムはどのように処理すべきですか？ → A: 起動前に厳密なバリデーション + 具体的なエラーメッセージで停止
- Q: バックテストのパフォーマンス目標は、正式な要件として定義すべきですか？ → A: 不要（パフォーマンス目標は削除）
- Q: 複数銘柄のバックテストで、一部の銘柄のみデータが不足している場合、システムはどのように処理すべきですか？ → A: 当日に存在する銘柄群を自然にユニバースとする。警告は出さない
...
- **FR-004**: システムは、iteration内で以下のステップを分離して実装できなければならない：
  ...
  - 銘柄選定（select_symbols）: ユーザが抽象基底クラス`BaseSymbolSelector`を継承し、Pydanticモデルでパラメータを定義し、`select()`メソッドをオーバーライドして実装。入出力はPolars DataFrameを使用。デフォルト実装（シグナル上位N銘柄選定）を提供
  - 執行条件計算（create_orders）: ユーザが抽象基底クラス`BaseOrderCreator`を継承し、Pydanticモデルでパラメータを定義し、`create()`メソッドをオーバーライドして実装。入出力はPolars DataFrameを使用。デフォルト実装（等ウェイトポートフォリオ、成行注文）を提供
  - 執行（submit_orders）
...
- **FR-007-1**: システムは、各iterationにおいて、当日にデータが存在する銘柄のみをユニバース（対象銘柄群）として自動的に決定しなければならない。一部の銘柄のデータが不足している場合でも、警告を出さず、データが存在する銘柄のみで処理を継続しなければならない
...
- **FR-014**: システムは、リターン計算ロジックを抽象基底クラス`BaseReturnCalculator`として定義し、ユーザが`calculate()`メソッドをオーバーライドして実装できなければならない。この計算結果をシグナルと結合して順位相関係数を評価できなければならない
...
- **FR-018**: システムは、ポジション情報、選択銘柄、モデルパラメータなどのコンテキストを、JSON、Parquetから復元可能な形式で保存・読み込みできなければならない
- **FR-019**: システムは、バックテスト時はローカルファイル、実運用時はS3やデータベースなど、コンテキストの保存先を切り替え可能でなければならない
...
- **FR-023**: システムは、データソースの定義、コストパラメータ、ループ管理設定をtomlファイルで管理しなければならない
...
- **Signal（シグナル）**: 各銘柄に対する定量的な特徴量または予測値。`BaseSignalCalculator`を継承したクラスの`calculate()`メソッドから生成され、銘柄選定の基礎となる。
- **BaseSignalCalculator（シグナル計算抽象基底クラス）**: ユーザがシグナル計算ロジックを実装するための抽象基底クラス。Pydanticモデルでパラメータを受け取り、`calculate()`メソッドをオーバーライドして、複数のデータソースを入力として受け取り、シグナルを返す。返り値のスキーマ（datetime, symbol, signal列）は実行時バリデーションされる。
- **BaseSymbolSelector（銘柄選定抽象基底クラス）**: ユーザが銘柄選定ロジックを実装するための抽象基底クラス。Pydanticモデルでパラメータを受け取り、`select()`メソッドをオーバーライドして、シグナルDataFrameから銘柄リストを返す。デフォルト実装（シグナル上位N銘柄選定）を提供。
- **BaseOrderCreator（注文生成抽象基底クラス）**: ユーザが注文生成ロジックを実装するための抽象基底クラス。Pydanticモデルでパラメータを受け取り、`create()`メソッドをオーバーライドして、シグナル、選定銘柄、ポジションから注文DataFrameを返す。デフォルト実装（等ウェイトポートフォリオ、成行注文）を提供。
- **BaseReturnCalculator（リターン計算抽象基底クラス）**: ユーザがリターン計算ロジックを実装するための抽象基底クラス。`calculate()`メソッドをオーバーライドして、市場データを入力として受け取り、リターン系列を返す。シグナル評価時に使用される。返り値の列スキーマは実行時バリデーションされる。
...
- **Context（コンテキスト）**: iterationをまたいで保持される状態（ポジション情報、選択銘柄、モデルパラメータなど）。各メソッドが参照・更新し、永続化される。
...
- **取引所APIエラーハンドリング**: 実運用時のAPI呼び出しにおけるエラーハンドリング（リトライ、タイムアウト、レート制限対策など）は、ユーザがAPIラッパーをオーバーライドする際に実装する責任を持つ。コアシステムは標準的なエラーハンドリング機能を提供しない
