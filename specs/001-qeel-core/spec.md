...
- **FR-022**: システムは、iteration内の各ステップの出力（シグナル、ポートフォリオ計画、注文）を個別に日付ごとにパーティショニングして保存しなければならない。ポジション情報は約定履歴から計算されるため保存対象外。年月でディレクトリ分割し、トレーサビリティを確保する。読み込み時は、最新日付または指定日付の各要素を復元し、ポジションは取引所APIから取得、またはモック約定履歴から計算してコンテキストを構築できなければならない
...
- **Portfolio Plan**: ポートフォリオ構築計画。シグナル、現在のポジションから計算された選択銘柄とメタデータ（シグナル強度、優先度、タグ等）を含む。執行条件計算の入力となる
