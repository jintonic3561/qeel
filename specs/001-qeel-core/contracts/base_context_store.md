...
コンテキスト（シグナル、ポートフォリオ計画、注文）の保存・読み込みを抽象化するインターフェース。ポジション情報は`BaseExchangeClient`から動的に取得されるため、保存対象外。バックテスト時はローカルファイル、実運用時はS3やデータベースを使用する。
...
class BaseContextStore(ABC):
    ...
    各ステップの出力（signals, portfolio_plan, orders）を個別に保存することで、
    ...
    def load(self, target_datetime: datetime, exchange_client: "BaseExchangeClient") -> Context | None:
        ...
        Returns:
            保存されたコンテキスト。指定日付で利用可能な要素（signals, portfolio_plan, orders）を
...
class LocalStore(BaseContextStore):
    ...
    def load(self, target_datetime: datetime, exchange_client) -> Context | None:
        ...
        # いずれかの保存要素が存在する場合のみContextを構築
        if any([signals is not None, portfolio_plan is not None, orders is not None]):
            return Context(
...
### save_signals / save_portfolio_plan / save_orders
...
- 入力: `target_datetime: datetime`, `exchange_client: BaseExchangeClient`
- 出力: 保存された `Context` またはNone
- 指定日付で利用可能な要素（signals, portfolio_plan, orders）をストレージから読み込み、current_positionsは`exchange_client.fetch_positions()`から取得してContextに格納
- 保存された要素が存在しない場合はNone
- 破損している場合はRuntimeErrorをraise

### load_latest

- 入力: `exchange_client: BaseExchangeClient`
- 出力: 最新の `Context` またはNone
- 内部で`_find_latest_datetime()`を呼び出して最新日付を取得し、`load(target_datetime, exchange_client)`に委譲する
- コンテキストが存在しない場合はNone
- 破損している場合はRuntimeErrorをraise
...
各実装は以下のメソッドを提供する：
- `save_signals()`, `save_portfolio_plan()`, `save_orders()`: iteration内の各ステップ出力を個別に保存
- `load(target_datetime, exchange_client)`: 指定日付のコンテキストを読み込み、ポジションはexchange_clientから取得
- `load_latest(exchange_client)`: 最新日付のコンテキストを読み込み、ポジションはexchange_clientから取得
