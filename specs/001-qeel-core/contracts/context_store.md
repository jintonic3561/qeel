...
コンテキスト（シグナル、ポートフォリオ計画、注文）の保存・読み込みを担当する単一実装クラス。ポジション情報は`BaseExchangeClient`から動的に取得されるため、保存対象外。IOレイヤー（BaseIO）を経由してデータ読み書きを行い、データ参照先の判別はIOレイヤーに委譲する。
...
class ContextStore:
    ...
    def save_orders(self, target_datetime: datetime, orders: pl.DataFrame) -> None:
        ...
        partition_dir = self.io.get_partition_dir(self.base_path, target_datetime)
        date_str = target_datetime.strftime("%Y-%m-%d")
        path = f"{partition_dir}/orders_{date_str}.parquet"
        self.io.save(path, orders, format="parquet")
    ...
    def load(self, target_datetime: datetime, exchange_client: "BaseExchangeClient") -> Context | None:
        ...
        orders = self.io.load(f"{partition_dir}/orders_{date_str}.parquet", format="parquet")
    ...
    def exists(self, target_datetime: datetime) -> bool:
        ...
        return any([
            ...
            self.io.exists(f"{partition_dir}/orders_{date_str}.parquet"),
        ])
