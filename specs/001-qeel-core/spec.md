...
- **FR-018**: システムは、iteration内の各ステップの出力（シグナル、ポートフォリオ計画、執行条件、ポジション情報）を個別に日付ごとにパーティショニングして保存しなければならない。DataFrameはParquet形式、その他のメタデータはJSON形式で保存される。年月でディレクトリ分割し（YYYY/MM/）、トレーサビリティを確保する。読み込み時は、最新日付または指定日付の各要素を復元し、存在しない要素はNoneとしてコンテキストを構築できなければならない
