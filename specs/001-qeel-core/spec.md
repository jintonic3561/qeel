...
- **FR-026**: システムは、実運用時のExecutor実装を支援するため、`qeel.utils`でヘルパー機能を提供しなければならない。ユーザはこれらを自由に利用可能だが、利用は強制ではない
...
- **APIエラーハンドリング**: 実運用時の取引所API呼び出しにおけるエラーハンドリング（リトライ、タイムアウト、レート制限対策、通知など）は、ユーザがExecutor実装時に独自に実装する責任を持つ。システムは`qeel.utils`でヘルパー機能を提供するが、その利用はユーザの選択に委ねられる
