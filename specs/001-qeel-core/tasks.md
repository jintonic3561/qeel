...
- [ ] T145 src/qeel/core/ディレクトリと__init__.pyを作成
- [ ] T146 [P] tests/unit/test_strategy_engine.pyを作成（空のテストファイル）
- [ ] T147 [P] tests/integration/test_strategy_engine_integration.pyを作成（空のテストファイル）
...
- [ ] T148 ステップ名EnumのRed: tests/unit/test_strategy_engine.pyにStepName Enumのテストを作成
...
- [ ] T149 ステップ名EnumのGreen: src/qeel/core/strategy_engine.pyにStepName Enumを実装
...
- [ ] T150 StrategyEngine初期化のRed: tests/unit/test_strategy_engine.pyに初期化テストを作成
...
- [ ] T151 StrategyEngine初期化のGreen: src/qeel/core/strategy_engine.pyにStrategyEngineクラスの__init__を実装
...
- [ ] T152 calculate_signalsステップのRed: テストを作成
...
- [ ] T153 calculate_signalsステップのGreen: StrategyEngine._run_calculate_signals()を実装
...
- [ ] T154 construct_portfolioステップのRed: テストを作成
...
- [ ] T155 construct_portfolioステップのGreen: StrategyEngine._run_construct_portfolio()を実装
  - Contextからsignalsを取得
...
- [ ] T156 create_entry_ordersステップのRed: テストを作成
...
- [ ] T157 create_entry_ordersステップのGreen: StrategyEngine._run_create_entry_orders()を実装
...
- [ ] T158 create_exit_ordersステップのRed: テストを作成
...
- [ ] T159 create_exit_ordersステップのGreen: StrategyEngine._run_create_exit_orders()を実装
  - exchange_client.fetch_positions()でポジションを取得（最新状態を保証するため毎回呼び出す）
...
- [ ] T166 run_stepsのRed: tests/unit/test_strategy_engine.pyにrun_stepsテストを作成
  - run_steps(date, [step1, step2, step3])が各ステップを順番に実行することを検証
  - 空のリストを渡した場合は何も実行しないことを検証

- [ ] T167 run_stepsのGreen: StrategyEngine.run_steps()を実装
...
- [ ] T169 run_all_stepsのGreen: StrategyEngine.run_all_steps()を実装
...
- [ ] T171 データ取得期間計算のGreen: StrategyEngine._get_data_fetch_range()を実装
...
- [ ] T176 統合テストのセットアップ: tests/integration/test_strategy_engine_integration.pyにフィクスチャを作成
  - MockDataSource、MockExchangeClient、InMemoryStore、サンプルSignalCalculator等のフィクスチャ

- [ ] T177 [P] 統合テスト: calculate_signalsからsubmit_ordersまでの一連の流れをテスト
...
- [ ] T179 [P] 統合テスト: コンテキスト復元とステップ独立実行テスト（FR-012対応）
  - シナリオA: 部分実行→復元→継続
...
- [ ] T180 src/qeel/core/__init__.pyにStrategyEngine, StepName, StrategyEngineErrorをエクスポート
- [ ] T181 src/qeel/__init__.pyにcoreモジュールをエクスポート
- [ ] T186 [P] 技術的負債解消: ExchangeClientProtocolの削除
...
- [ ] T182 全テストがパスすることを確認: `uv run pytest tests/unit/test_strategy_engine.py tests/integration/test_strategy_engine_integration.py -v`
- [ ] T183 mypyチェック: `uv run mypy src/qeel/core/ src/qeel/stores/`
- [ ] T184 ruffチェック: `uv run ruff check src/qeel/core/ src/qeel/stores/ tests/unit/test_strategy_engine.py tests/integration/test_strategy_engine_integration.py`
- [ ] T185 ruff format: `uv run ruff format src/qeel/core/ src/qeel/stores/ tests/unit/test_strategy_engine.py tests/integration/test_strategy_engine_integration.py`
