...
**Independent Test**: バックテストで使用した同一のシグナル計算ロジックと執行ロジックを用い、外部スケジューラから各ステップを独立して実行し、実際の取引所API経由で注文が送信されることで検証可能。

**Acceptance Scenarios**:

1. **Given** バックテストで検証済みの戦略ロジックが存在する、**When** 実運用で各ステップをそれぞれ実行する、**Then** 取引所APIを通じて実際の注文が送信される
...
クオンツアナリストが、シグナル計算ロジックを指定期間のデータに対して一括実行し、シグナルの有効性をリターンとの順位相関係数で評価する。
...
- **FR-004**: システムは、iteration内で以下のステップを分離して実装できなければならない：
  ...
  - ポートフォリオ構築: ユーザがカスタムロジックを実装可能。シグナルを入力として受け取り、ポートフォリオ計画を出力。出力にはエントリー注文計算に必要なメタデータ（シグナル強度、優先度、タグ等）を含められる。デフォルト実装（シグナル上位N銘柄選定）を提供
  - エントリー注文生成: ユーザがカスタムロジックを実装可能。ポートフォリオ計画（メタデータ含む）、現在のポジション、価格データを入力として受け取り、エントリー注文を生成。メタデータを活用して柔軟な注文生成が可能。デフォルト実装（等ウェイトポートフォリオ、成行注文）を提供
  - エグジット注文生成: ユーザがカスタムロジックを実装可能。現在のポジション、価格データを入力として受け取り、エグジット注文を生成。デフォルト実装（保有ポジションの成行全決済）を提供
  - エントリー注文執行: 生成されたエントリー注文を実行
  - エグジット注文執行: 生成されたエグジット注文を実行
  - 約定情報取得: 約定結果を取得
...
- **FR-006**: システムは、ポジション情報、選択銘柄、シグナルなどのコンテキストを各ステップ間で永続化し、次のステップまたは次のiterationで参照できなければならない。これにより、各ステップを独立して実行可能にし、ステップ間での状態受け渡しを保証する
...
- **FR-012**: システムは、iteration内の各ステップを独立して実行可能でなければならない。実運用時は外部スケジューラから各ステップをそれぞれ呼び出し、バックテストと完全な再現性を保証しなければならない
...
- **FR-016**: システムは、シグナル計算ロジックに対して複数の可能なパラメータの組を設定し、指定期間のデータに対して一括でシグナルを計算できなければならない
...
- **FR-019**: システムは、各データソースのパラメータ（日時列名、利用可能時刻のオフセット、提供window、データソースクラスのモジュール・クラス名）を設定で管理しなければならない。設定ファイルでデータソースクラスを指定することで、コード変更なしにデータソースを追加・変更できる。ただし、データソースには必ずohlcvデータを含まなければならない
- **FR-020**: システムは、データソースの読み込み元をユーザが柔軟に定義できるインターフェースを提供しなければならない（標準ファイル形式、データベース、外部APIなど）。また、設定ファイルから全データソースを一括ロードする機能を提供しなければならない
- **FR-021**: システムは、データソースに欠損値が含まれる場合、そのまま処理を継続しなければならない。欠損値処理は、ユーザがシグナル計算ロジック内で実装する責任を持つ

#### コンテキスト管理

- **FR-022**: システムは、iteration内の各ステップの出力（シグナル、ポートフォリオ計画、エントリー注文、エグジット注文）を個別に日付ごとにパーティショニングして保存しなければならない。ポジション情報は約定履歴から計算されるため保存対象外。年月でディレクトリ分割し、トレーサビリティを確保する。読み込み時は、最新日付または指定日付の各要素を復元し、ポジションは取引所APIから取得、またはモック約定履歴から計算してコンテキストを構築できなければならない
- **FR-023**: システムは、設定で指定されたストレージタイプ（ローカル/クラウドストレージ）に基づき、自動的にコンテキストの保存先を判断しなければならない

#### 結果分析と可視化

- **FR-024**: システムは、約定情報から共通のパフォーマンス指標（シャープレシオ、最大ドローダウン、勝率、日次リターンなど）を計算、可視化できなければならない
- **FR-025**: システムは、バックテスト結果と実運用結果の乖離を可視化できなければならない
- **FR-026**: システムは、期間を指定してバックテスト・実運用問わず共通のレポート出力ができなければならない

#### 設定管理

- **FR-027**: システムは、全体設定（戦略名、ストレージタイプ、クラウドストレージ設定）、データソースの定義、コストパラメータ、ループ管理設定を設定ファイルで管理しなければならない。全体設定は、データソースやコンテキストの保存参照先の情報を含む
- **FR-028**: システムは、各ステップ（シグナル計算、銘柄選定、執行など）がバックテストで呼ばれるタイミングを、ループ日時からのオフセットとして設定で定義できなければならない
...
- **FR-032**: システムは、環境変数でワークスペースディレクトリを指定可能でなければならない。未設定時はカレントディレクトリを参照する。ワークスペース配下には設定、入力データ、出力データのディレクトリが配置される。入出力データは、ストレージタイプがローカルの場合に使用される
...
- **FR-034**: システムは、ファイル読み書きを抽象化するストレージ層を提供しなければならない。ストレージ層は以下の責務を持つ:
  ...
  - データおよびコンテキストの形式に対応したデータの保存・読み込みメソッド
...
- **Portfolio Plan**: ポートフォリオ構築計画。シグナル、現在のポジションから計算された選択銘柄とメタデータ（シグナル強度、優先度、タグ等）を含む。エントリー注文計算の入力となる
- **Order（注文）**: 執行する注文の詳細（銘柄、売買区分、数量、価格、注文タイプなど）。ポートフォリオ計画、現在のポジション、価格データから計算され、執行ステップで実行される
- **Position（ポジション）**: 現在保有している銘柄と数量。各iterationで参照され、ポートフォリオ計画計算、エントリー注文計算、エグジット注文計算の入力となる
- **Fill Report（約定情報）**: 注文が約定した結果の詳細（注文ID、銘柄、売買区分、数量、価格、手数料、タイムスタンプ等）。パフォーマンス計算の入力となる
- **Context（コンテキスト）**: iterationをまたいで保持される状態（現在日時、シグナル、ポートフォリオ計画、エントリー注文、エグジット注文、ポジション情報など）。各ステップが参照・更新し、永続化される
...
- **SC-005**: シグナル評価において、複数のパラメータの組に対して順位相関係数の分布が可視化され、オーバーフィットの有無を判断できる
...
- **リターン計算の標準化**: リターンは対数リターンを標準実装として提供する。単純リターンやより複雑なリターン計算（例: リスク調整後リターン、ベンチマーク相対リターン）はユーザ側で拡張可能とする
...
- **ステップ実行モデル**: バックテストと実運用で同一のエンジンを使用する。バックテストはループ管理機能がエンジンのステップ実行メソッドを繰り返し呼び出し、実運用は外部スケジューラがステップ実行メソッドをそれぞれ独立に呼び出す。各ステップ間ではコンテキストを永続化し、状態を受け渡す
...
- **シングルユーザ**: 複数ユーザの同時アクセスや権限管理は対象外とする