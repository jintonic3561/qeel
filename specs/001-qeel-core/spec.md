...
- **バックテストと実運用の分離**: バックテストと実運用は通常、別々のスクリプトで実行される。ユーザはコード内で`BacktestEngine`または`LiveEngine`を明示的にインスタンス化し、ExchangeClient、DataSource等の依存を注入する（Dependency Injection）。同一スクリプトで設定ファイルのみでモードを切り替えたい場合、ユーザがファクトリー関数を実装可能
...
- **モード設定による自動切り替え**: システムは、toml設定や環境変数によるバックテスト/実運用の自動切り替え機構を提供しない。ユーザがコンストラクタで明示的にEngineを選択する設計を採用し、依存関係の透明性とテスタビリティを優先する
